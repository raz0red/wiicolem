...
/** Windows using TCP/IP via sockets.                       **/
/**                                                         **/
...
int NETConnect(const char *Server,unsigned short Port)
{
  ...
  if(Server)
  {
    ...
    if(connect(Socket,(struct sockaddr *)&Addr,sizeof(Addr))>=0)
    {
      ...
      J=!Blocking;
      if(ioctlsocket(Socket,FIONBIO,&J)<0)
      { closesocket(Socket);Socket=-1;return(NET_OFF); }
  ...
  { closesocket(LSocket);return(NET_OFF); }

  /* Make listening socket non-blocking */
  J=1;
  if(ioctlsocket(LSocket,FIONBIO,&J)<0)
  { closesocket(LSocket);return(NET_OFF); }
  ...
  do
  {
    ...
    if(WSAGetLastError()&&(WSAGetLastError()!=WSAEWOULDBLOCK)) break;
  }
...
int NETBlock(int Switch)
{
  ...
  {
    J=!Switch;
    if((Socket<0)||(ioctlsocket(Socket,FIONBIO,&J)>=0)) Blocking=Switch;
  }
