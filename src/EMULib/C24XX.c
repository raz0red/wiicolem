...
/** Reset the 24xx chip.                                    **/
/*************************************************************/
void Reset24XX(C24XX *D,byte *Data,unsigned int Flags)
{
...
{
//  if(D->Flags&C24XX_DEBUG)
//    printf("EEPROM SDA=%d, SCL=%d\n",D->Pins&C24XX_SDA? 1:0,D->Pins&C24XX_SCL? 1:0);
...
//  if((D->Flags&C24XX_DEBUG)&&((D->Pins^V)&(C24XX_SDA|C24XX_SCL)))
//    printf("EEPROM SDA: %d=>%d, SCL: %d=>%d\n",
//      D->Pins&C24XX_SDA? 1:0,V&C24XX_SDA? 1:0,
//      D->Pins&C24XX_SCL? 1:0,V&C24XX_SCL? 1:0
//    );
