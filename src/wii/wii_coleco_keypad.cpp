 ...
 *
 * @param   pad The pad to render the description for
 * @param   clear Are we clearing the description?
 */
static void render_description(keypad* pad) {
    char* desc = wii_coleco_db_get_button_description(
    ...
    wii_gx_drawrectangle(x + -padding, y + h + padding, w + (padding << 1),
                         h + (padding << 1), (GXColor){0x0, 0x0, 0x0, 0x80},
                         TRUE);
 ...
 * Renders the keypad
 *
 * @param   pad The keypad to render
 */
static void render_pad(keypad* pad) {
    if (pad->visible) {
        ...
        color = (GXColor){overlay->keypSelKeyRgba.R, overlay->keypSelKeyRgba.G,
                          overlay->keypSelKeyRgba.B, overlay->keypSelKeyRgba.A};

        if (idata != NULL) {
            int padx =
                (pad == &keypad0 ? PAD_X + -WII_WIDTH_DIV2
                                 : WII_WIDTH_DIV2 - PAD_X - idata->width);
            int pady = WII_HEIGHT_DIV2 - PAD_Y;

            wii_gx_drawimage(padx, pady, idata->width, idata->height,
                             idata->data, 0, 1.0, 1.0, 0xFF);

            wii_gx_drawrectangle(
                padx + offsetx + ((pad->selected_key % 3) * (keyw + gapx)),
                pady - offsety - ((pad->selected_key / 3) * (keyh + gapy)),
 ...
 *
 * @param   pad The keypad to update
 * @param   joy The current joystick input
...
static unsigned int update_keypad(keypad* pad, byte joy) {
    if (pad->joy != JOY_NONE && joy != JOY_SELECT) {
        // Ignore subsequent joystick events
        // (simulate button down event)
        return 0;
    }
 ...
 *
 * @param   joyIndex The joystick to poll (0-based)
 * @param   pad The keypad associated with the specified joystick
 * @return  State indicating keypad selection   
 */
static unsigned int poll_joystick(int joyIndex, keypad* pad) {
    ...
    if (pad->visible) {
        ...
        // Select
        if ((held & (WII_BUTTON_CV_1 | WII_BUTTON_CV_2 | WII_BUTTON_CV_3 |
                     WII_BUTTON_CV_4 |
                     (isClassic ? (WII_CLASSIC_CV_1 | WII_CLASSIC_CV_2)
...
BOOL wii_keypad_is_visible(int keypadIndex) {
    return keypadIndex == 0 ? keypad0.visible : keypad1.visible;
}
...
u32 wii_keypad_poll_joysticks() {
    if (is_swapped()) {
        return poll_joystick(0, &keypad0) << 16;
    } else {
        return poll_joystick(0, &keypad0) | poll_joystick(1, &keypad1) << 16;
    }
}
...
void wii_keypad_render() {
    if (small_keypad_idata == NULL) {
        small_keypad_idata = wii_gx_loadimagefrombuff(keypad_small_png);
    }

    if (med_keypad_idata == NULL) {
        med_keypad_idata = wii_gx_loadimagefrombuff(keypad_med_png);
    }

    if (large_keypad_idata == NULL) {
        large_keypad_idata = wii_gx_loadimagefrombuff(keypad_large_png);
    }
    ...
    if (dim_screen) {
        wii_gx_drawrectangle(-WII_WIDTH_DIV2, WII_HEIGHT_DIV2, WII_WIDTH,
                             WII_HEIGHT, (GXColor){0x0, 0x0, 0x0, 0x80}, TRUE);
    }
