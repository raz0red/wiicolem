...
static char* get_db_path()
{
  if( db_file[0] == '\0' )
  {
    snprintf( db_file, WII_MAX_PATH, "%s%s", wii_get_fs_prefix(), DB_FILE_PATH );
  }
...
static char* get_db_tmp_path()
{
  if( db_tmp_file[0] == '\0' )
  {
    snprintf( db_tmp_file, WII_MAX_PATH, "%s%s", wii_get_fs_prefix(), DB_TMP_FILE_PATH );
  }
...
static char* get_db_old_path()
{
  if( db_old_file[0] == '\0' )
  {
    snprintf( db_old_file, WII_MAX_PATH, "%s%s", wii_get_fs_prefix(), DB_OLD_FILE_PATH );
  }
...
int wii_coleco_db_is_button_available( u16 index, u8 mode )
{
  ...
  switch( mode )
  {
    ...
    case CONTROLS_MODE_STANDARD:
      return 
        buttonValue != JST_PURPLE &&
        buttonValue != JST_BLUE &&
        buttonValue != JST_2P_FIRER &&
        buttonValue != JST_2P_FIREL;
    case CONTROLS_MODE_ROLLER:
      return 
        buttonValue != JST_PURPLE &&
        buttonValue != JST_BLUE;
      ...
      if( read_mode == 1 )
      {
        ...
        if( ptr )
        {
          *ptr++ = '\0';
          ...
          {
            snprintf( button, 
              sizeof(button), "buttonDesc%d", ButtonNames[i].button );
            if( !strcmp( buff, button ) )
            {
