...
void wii_coleco_menu_init() {
    ...
    child =
        wii_create_tree_node(NODETYPE_CARTRIDGE_SETTINGS_CURRENT_SPACER, "");
    wii_add_child(wii_menu_root, child);
    ...
    child =
        wii_create_tree_node(NODETYPE_SAVE_CARTRIDGE_SETTINGS, "Save settings");
    wii_add_child(cart_settings, child);

    child = wii_create_tree_node(NODETYPE_REVERT_CARTRIDGE_SETTINGS,
                                 "Revert to saved settings");
    wii_add_child(cart_settings, child);

    child = wii_create_tree_node(NODETYPE_DELETE_CARTRIDGE_SETTINGS,
                                 "Delete settings");
    wii_add_child(cart_settings, child);
 ...
 *
 * @param   menu The menu
 * @param   buffer The buffer to update with the header message for the
 *          current menu.
 */
...
void wii_menu_handle_get_node_name(TREENODE* node, char* buffer, char* value) {
    ...
    switch (node->node_type) {
        ...
        case NODETYPE_KEYPAD_SIZE_CART: {
            int size = (node->node_type == NODETYPE_KEYPAD_SIZE_CART
        ...
        case NODETYPE_BUTTON8: {
            ...
            wii_coleco_db_get_button_name(btnName, sizeof(btnName),
